...
6. Since the "from" email domain doesn't match the domain that actually sent the email, it's marked as spam. _Maybe_.
...
- `include:spf.messagingengine.com`: Emails are allowed to be sent from `spf.messagingengine.com`. This is the sending server for Fastmail. There's also one for Mandrill, for transactional emails. You can have as many of these as you want.
...
- `rua=mailto:postmaster@simonandrews.ca` tells an email server to send **aggregate** reports of spoofed emails to `postmaster@simonandrews.ca`
- `ruf=mailto:postmaster@simonandrews.ca` tells an email server to send **forensic** reports of spoofed emails to `postmaster@simonandrews.ca`
...
You'll find your feet here, with a bit of time.
...
export const meta = {
  title: "How to stop email spoofing on your domain",
  description:
    "Use SPF, DKIM, and DMARC to improve your domain's email security - but written for humans.",
}
